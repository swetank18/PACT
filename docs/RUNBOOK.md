...
| 3 | `1` | 4 s | An order, a decision, ten checks in order, none skipped | "End to end. Discovery, mandate, quote, gate, settlement — milliseconds, and every check is on screen with what it cost. Ten ran, none skipped: this one passed all the way down." |
...
| 6 | `4` | 2 s | Four blocks, four reason codes | "Replay, a merchant outside scope, a tampered amount, a prompt injection in a product description. Each one a machine-readable code, not prose — and the chain stops at the check that caught it, so the ones after it read as skipped." |
