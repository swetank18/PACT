...
| A cue lands late, or a staggered reveal finishes as the scene ends | the same frame dropping, but *inside* the scene: a page holding still between two cues commits no frames, so the recording compresses those gaps and the cut stretches them back unevenly. Measured on S04: cues written for 8.2–10.7s arrived at 16.3–23.4s of a 24.4s clip | already handled: every scene page carries a one-pixel `.ticker` rotating off-frame forever, so the compositor commits a frame every tick and the recording keeps real time. Remove it and the drift returns |
