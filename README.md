...
a request, so this is not serverless-deployable as it stands. `render.yaml` has
been run — `pact-9btr.onrender.com`, though on the free plan, which has no disk;
see *Limitations*. `fly.toml` is unexecuted.
...
| | |
| --- | --- |
| `ci` | 213 Python tests, 55 console tests, typecheck, and a contract-drift check |
