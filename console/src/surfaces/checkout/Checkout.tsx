...
export function Checkout({
  ...
  /** The quote whose order we are waiting to see arrive on the stream. */
  const awaitingQuoteId = useRef<string | null>(null);
  /** Set when the purchase in flight is replacing a rolled-back order. */
  const recoveringFrom = useRef<string | null>(null);
  const shownOrders = useRef<Set<string>>(new Set());
  ...
  async function authorizeAndOrder(q: Quote, addon?: Addon | null) {
    ...
    try {
      ...
      // The merchant acknowledges and runs the saga behind the response, so the
      // console can watch the steps arrive one at a time — which is exactly what
      // the failure demo needs. The order itself reaches us on the stream, and
      // `awaitingOrder` below picks it up.
      await merchant.createOrder(
        finalQuote.quote_id,
        decision.decision_id,
        decision.settlement_token ?? "",
        recoveringFrom.current ?? undefined,
      );
      recoveringFrom.current = null;
      awaitingQuoteId.current = finalQuote.quote_id;
      say({ role: "agent", text: "Paid. Watching it settle." });
      await refreshHeadroom();
    } catch (e) {
      replace(working, {
        role: "agent",
  ...
  /**
   * The order we just paid for arrives on the merchant stream rather than in
   * the response. Match it by quote id and put it in the thread with its
   * timeline, so the conversation shows the settlement rather than stopping at
   * "paid".
   */
  useEffect(() => {
    const waiting = awaitingQuoteId.current;
    if (!waiting) return;
    const order = orders.find((o) => o.quote_id === waiting);
    if (!order || shownOrders.current.has(order.order_id)) return;

    shownOrders.current.add(order.order_id);
    awaitingQuoteId.current = null;
    onWatchOrder(order.order_id);
    say({ role: "card", card: { kind: "order", order } });
    say({ role: "card", card: { kind: "saga", orderId: order.order_id } });
  }, [orders, onWatchOrder, say]);
  ...
  async function acceptAlternative(orderId: string) {
    ...
    try {
      // The merchant hands back a quote, not an order. It has no key and cannot
      // spend for us, so the replacement is signed here and goes through the
      // gate like any other purchase — which is why the recovery is still
      // bounded by the mandate.
      const { quote: replacement, recovered_from } = await merchant.acceptAlternative(orderId);
      recoveringFrom.current = recovered_from;
      setQuote(replacement);
      say({ role: "card", card: { kind: "quote", quote: replacement } });
      await authorizeAndOrder(replacement);
    } catch (e) {
      say({
        role: "agent",
                ...
                {c.kind === "order" && (
                  <div className={s.orderDone}>
                    ...
                    <div className={s.orderMeta}>
                      ...
                      {c.order.rail_payment_id && <span>{c.order.rail_payment_id}</span>}
