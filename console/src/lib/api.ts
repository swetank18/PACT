...
export const merchant = {
   ...
   *
   * Returns an acknowledgement, not the order. The saga runs behind the
   * response so the console can watch the steps arrive one at a time, which is
   * what the failure demo needs — the order itself arrives on the stream.
   */
  createOrder: (
    quoteId: string,
    decisionId: string,
    ...
    post<{ accepted: boolean; decision_id: string; quote_id: string }>(
      `${MERCHANT}/v1/orders`,
      {
        quote_id: quoteId,
  ...
  /**
   * Takes the alternative offered after a rollback.
   *
   * Returns a replacement **quote**, not a completed order. The merchant holds
   * no key and cannot spend on the buyer's behalf, so the buyer signs a fresh
   * authorize against this quote — which is the only reason the recovery is
   * still bounded by the mandate.
   */
  acceptAlternative: (orderId: string) =>
    post<{ quote: Quote; recovered_from: string; mandate_id: string }>(
      `${MERCHANT}/v1/orders/${orderId}/accept_alternative`,
    ),
